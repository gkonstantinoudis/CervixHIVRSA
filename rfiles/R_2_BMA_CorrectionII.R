...
# load data
dat <- readRDS("data/data_DataSouthAfrica_corII")
...
form <- CCcases ~ 1 + offset(log(HIVcases)) + f(id.time, model='rw1', hyper=hyper.iid, constr = TRUE, scale.model = TRUE) + 
...
# saveRDS(in.mod, file = "results/mod_CorrectionII")

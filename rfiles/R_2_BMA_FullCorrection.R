...
# Run best fitting model: Full Correction
...
listOV <- readRDS("data/data_DataSouthAfrica_fullcor")
...
model_FullCorrection <- function(X){
...
par.fun <- function(k){model_FullCorrection(X = k)}

clusterExport(cl_inla, c("model_FullCorrection", "listOV", "W.adj", 
...
model_FullCorrection <- inla.merge(outpar, prob = rep(1, times = 100))

# saveRDS(model_FullCorrection, file = "results/model_FullCorrection")
